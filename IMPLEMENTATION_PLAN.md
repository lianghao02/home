...
## 2026-08-31 00_home 目錄與入口整理檢視

### 目標與驗收條件

- 保持根目錄三個旗艦 BAT 與既有 PowerShell 入口相容，不為外觀整齊搬動仍被呼叫的檔案。
- 確認所有管理腳本可由 Windows PowerShell 5.1 正確解析，避免 UTF-8 編碼造成啟動失敗。
- 僅提出已不再使用的 Banner 與舊下載頁整理建議；未獲明確授權前不搬移或刪除。

### 已確認決策

- 根目錄保留 `1_全專案智慧同步中樞.bat`、`2_建置所有桌面應用程式.bat`、`3_環境建置與工具安裝.bat`，作為日常操作入口。
- `build_all_desktop_apps.ps1` 與 `setup_all_envs.ps1` 保留於根目錄，分別由旗艦 BAT 與環境中樞腳本直接引用。
- 其餘管理腳本維持於 `scripts/`；不建立重複包裝器或移動檔案。

### 工作清單

- [x] 盤點根目錄、`scripts/`、`downloads/`、`backups/` 與 `images/` 的用途與引用關係｜確認根目錄結構已符合入口／維護程式／資源分層。
- [x] 檢查 8 支 PowerShell 與 3 支旗艦 BAT 的 UTF-8 BOM、入口關係與 Windows PowerShell 5.1 Parser｜全部通過。
- [x] 封存 13 組未引用的舊 JPG Banner｜使用者已確認退役；已移至 Git 忽略的 `backups\retired-banners-20260831\`，現行首頁仍使用 PNG。

### 風險與因應

- `photo_report.html` 與 `downloads/Photo_Report.rar` 互相引用，且可能仍有外部舊連結；未建立導向或退役通知前不移動。
- `cases.db` 雖為空檔且被 Git 忽略，但由 MCP 設定引用，必須保留。

---

## 2026-08-31 PowerShell 7 相容性與入口強化

### 目標與驗收條件

- 三個旗艦 BAT 優先使用已安裝的 PowerShell 7，未安裝時自動退回 Windows PowerShell 5.1。
- 中樞腳本呼叫子 PowerShell 時採相同選擇策略，避免外層與子腳本使用不同解譯器。
- 保留 UTF-8 BOM 與既有參數，確保舊電腦仍可使用。

### 不做範圍

- 不安裝 PowerShell、套件或其他工具鏈。
- 不變更同步、建置、發布或全域 Agent 設定的授權範圍。

### 工作清單

- [x] 更新三個旗艦 BAT 與相關中樞腳本的解譯器選擇｜兩種 PowerShell Parser 通過；環境中樞「0 離開」路徑實測未觸發任何變更。
- [x] 對齊 README 的 PowerShell 需求與後備行為｜入口檔與所有 `powershell.exe` 引用搜尋完成。
- [x] 驗證腳本語法、BOM 與 Git 差異｜PowerShell 5.1／7 Parser、UTF-8 BOM、`git diff --check` 與引用搜尋均通過。

---

## 2026-08-31 同步備份可還原性強化

### 目標與驗收條件

- 每個受管理目標的同步前備份都必須存入唯一且可辨識的目錄，避免同秒鐘內的同名檔案互相覆蓋。
- 保留既有 `-CheckOnly`、`-Force`、備份保留十組與正式同步行為，不執行任何全域部署。

### 工作清單

- [x] 以目標完整路徑產生安全的備份子目錄｜PowerShell 5.1／7 Parser、三組路徑樣本與 `sync_codex.ps1 -CheckOnly` 驗證通過，所有受管理項目皆為 Current。

### 風險與因應

- 舊版備份結構維持可讀；新備份只增加目標識別層級，清理規則仍以最上層工作階段目錄計數。
